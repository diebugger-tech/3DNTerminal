...
pub fn calculate_geometry(params: &TerminalParams) -> (Rectangle, f32) {
    match params.phase {
        AnimationPhase::Collapsed => {
            ...
            let hover = (time * 2.0).sin() * 4.0;
            let mut rect = params.corner_rect;
            rect.y += hover;
            (rect, 0.6) // Konstante Transparenz im Collapsed Modus
        }
...
pub fn draw(
    ...
    frame.fill_text(cosmic::iced::widget::canvas::Text {
        ...
        position: Point::new(rect.x + margin_x + 5.0, rect.y + margin_y + 2.0),
    ...
    if let Some(controls) = params.window_controls {
        ...
        let anchor = Point::new(rect.x + rect.width, rect.y); // Oben rechts am Rect
        controls.draw(frame, alpha, anchor, btn_size, params.active_corner, params.cursor_pos);
    }
