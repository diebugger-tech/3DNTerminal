...
pub fn get_quad(params: &HologramParams) -> [Point; 4] {
    /* 
       DO NOT DELETE - 3D HOLOGRAM LOGIC (Temporarily disabled for button stability)
    ...
    */
    ...
    // Stability Mode: Simple 2D Rect
    let (rect, _, _) = calculate_3d_geometry(params);
...
pub fn draw(
    ...
    let path = Path::rounded_rectangle(target_rect.position(), target_rect.size(), 4.0.into());
